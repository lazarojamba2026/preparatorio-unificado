...
const EXAMS = [
  {
    ...
    subtitulo: "História de Angola · Organização político-administrativa · Administração Pública · MININT · Patriotismo",
    accent: "#A23E2E",
    disciplinas: [
  ...
  },
  {
    ...
    disciplinas: [
      ...
      { id: "doc-historia", nome: "Docência — História", questoes: [
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!error && data) {
        setCodigosAcesso(data.map((c) => ({ codigo: c.codigo, titular: c.titular, criadoEm: new Date(c.criado_em).toLocaleDateString("pt-PT") })));
      }
      setCarregando(false);
    })();
  }, []);
  ...
  const entrarComCodigo = (codigoDigitado) => {
    ...
    if (valido) { setErro(""); setTela("inicio"); }
    else setErro("Código de acesso inválido. Confirme com o administrador.");
  };
  ...
  const entrarComoAdmin = async (senha) => {
    ...
    if (senha === senhaAtual) { setErro(""); setEhAdmin(true); setTela("admin"); }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F0E6D2", fontFamily: "'Source Serif 4', Georgia, serif", color: "#2A2620" }}>
      ...
      {!carregando && tela === "inicio" && (
        <TelaInicio onEscolher={escolherExame} ehAdmin={ehAdmin} onIrParaAdmin={() => setTela("admin")} onSair={() => { setEhAdmin(false); setTela("acesso"); }} />
      )}
...
function TelaInicio({ onEscolher, ehAdmin, onIrParaAdmin, onSair }) {
  return (
    <div style={{ maxWidth: 880, margin: "0 auto", padding: "40px 24px 80px" }}>
      ...
      <header style={{ borderBottom: "3px double #2A2620", paddingBottom: 24, marginBottom: 40 }}>
        ...
        <p className="sans" style={{ color: "#5C4A2E", fontSize: 16, marginTop: 12, maxWidth: 560 }}>Escolha um caderno de prova abaixo. Cada simulado é corrigido automaticamente, com gabarito comentado ao final.</p>
      </header>
...
function TelaAdmin({ codigos, onSalvarCodigos, onVoltar, adminPasswordKey, senhaPadrao }) {
  ...
  const adicionarCodigo = async () => {
    ...
    const { error } = await supabase.from("access_codes").insert({ codigo, titular: novoTitular.trim() || "—" });
    if (error) { setMsg("Não foi possível criar o código."); return; }
    onSalvarCodigos([...codigos, { codigo, titular: novoTitular.trim() || "—", criadoEm: new Date().toLocaleDateString("pt-PT") }]);
    setNovoCodigo(""); setNovoTitular("");
    setMsg(`Código "${codigo}" criado.`);
  };
    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 24px 100px" }}>
      ...
      <div style={{ background: "#FBF7EE", border: "1px solid #D8CBA8", borderRadius: 6, padding: 24, marginBottom: 24 }}>
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {codigos.map((c) => (
            <div key={c.codigo} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", background: "#F0E6D2", borderRadius: 4 }}>
              <div><span className="mono" style={{ fontWeight: 700, fontSize: 14, letterSpacing: "0.05em" }}>{c.codigo}</span><span className="sans" style={{ fontSize: 13, color: "#5C4A2E", marginLeft: 10 }}>{c.titular} · criado em {c.criadoEm}</span></div>
